...
[
  {
    ...
    try {
      ...
      const result = await geminiService.generate(prompt, 'boqDrawingExtraction', userId, '/api/boqs/upload-drawing', true, imagePart);
      if (Array.isArray(result) && result.length > 0) extractedItems = result;
    } catch (e) {}
...
[
  {
    ...
    try {
      const result = await geminiService.generate(prompt, 'boqGenerateFromBrief', userId, '/api/boqs/from-brief', true);
      if (Array.isArray(result) && result.length > 0) generatedItems = result;
    } catch (e) {}
...
Include material quality, standard dimensions/specs, and finish quality.`;
    ...
    try {
      const result = await geminiService.generate(prompt, 'boqEnhanceDescription', userId, '/api/boqs');
...
Return valid JSON:
{
  ...
  "warnings": ["Warning 1"]
    ...
    try {
      const result = await geminiService.generate(prompt, 'boqMissingItems', userId, '/api/boqs/analysis', true);
...
[
  { "suggestion": "Clear advice", "savings": 12000, "impact": "low|medium|high" }
    ...
    try {
      const result = await geminiService.generate(prompt, 'boqCostOptimizations', userId, '/api/boqs/analysis', true);
...
module.exports = new GeminiBoqService();
