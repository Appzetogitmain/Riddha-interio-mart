...
[
  { "suggestion": "Clear actionable advice", "savings": 15000, "impact": "low|medium|high" }
    ...
    try {
      const result = await geminiService.generate(prompt, 'estimatorOptimizations', userId, '/api/estimates', true);
...
Write a 2-sentence trade-off analysis explaining how the project timeline affects vendor lead times, labor costs, and overall savings.`;
    ...
    try {
      const result = await geminiService.generate(prompt, 'estimatorTimelineImpact', userId, '/api/estimates');
      return result || `Selecting a ${timeline} timeline applies a cost adjustment of ₹${timelineAdjustment.toLocaleString()}. Planning ahead allows vendors to source materials at standard rates without incurring expedited shipping or overtime labor charges.`;
    } catch (e) {
...
module.exports = new GeminiEstimatorService();
