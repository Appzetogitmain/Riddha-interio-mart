...
// fallback. Gemini here typically takes 5-20s — far past the sub-2s budget the
...
/**
 * Gemini-backed journey guidance (Requirement #17).
 *
 * Every method degrades to a deterministic fallback so the journey UI keeps
 * working when GEMINI_API_KEY is unset, the API is slow, or it is unavailable —
 * guidance is an enhancement, never a hard dependency.
 */
class GeminiJourneyService {
  ...
  /**
   * Stale-while-revalidate around a Gemini call.
   *
   ...
   *
   * `transform` shapes/validates the raw model output before it is cached.
   */
  ...
  /**
   * GEMINI PROMPT 1 — Next step guidance.
...
`;
    ...
    try {
      const guidance = await this._cachedOrBackfill({
        cacheKey,
        buildCall: () => geminiService.generate(prompt, 'journeyGuidance', userId, '/api/journey/next-suggestion', true),
      ...
      console.error('[GEMINI JOURNEY GUIDANCE ERROR]', err.message);
      return fallback;
    }
  }

  /**
   * GEMINI PROMPT 2 — Contextual help for the page the user is on.
   */
...
`;
    ...
    try {
      const help = await this._cachedOrBackfill({
        cacheKey,
        buildCall: () => geminiService.generate(prompt, 'journeyContextHelp', userId, '/api/journey/context-help', true),
        transform: (raw) => {
          if (!raw) return null;
      ...
      console.error('[GEMINI JOURNEY HELP ERROR]', err.message);
      return { ...FALLBACKS.journeyContextHelp };
    }
  }

  /**
   * GEMINI PROMPT 3 — Intelligent upsell / cross-sell, but only when it genuinely fits.
   */
...
`;
    ...
    try {
      const suggestion = await this._cachedOrBackfill({
        cacheKey,
        buildCall: () => geminiService.generate(prompt, 'journeyUpsell', userId, '/api/journey/next-suggestion', true),
        transform: (raw) => (raw ? {
      ...
      console.error('[GEMINI JOURNEY UPSELL ERROR]', err.message);
...
module.exports = new GeminiJourneyService();
