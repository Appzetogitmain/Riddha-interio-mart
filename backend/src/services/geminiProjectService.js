...
Respond in plain text with 2-3 professional, actionable sentences outlining progress, current stability, and momentum.`;
    ...
    try {
      const result = await geminiService.generate(prompt, 'projectHealthNarrative', userId, '/api/projects/health');
      return result || `Project "${projectName}" is currently ${completionPercentage}% complete and running ${overallStatus.replace('-', ' ')}. Budget utilization sits at ${budgetPercent}%, with ${completedPhases} of ${totalPhases} project phases finished successfully.`;
    } catch (e) {
...
Provide 3-5 clear, numbered next steps with brief rationale.`;
    ...
    try {
      const result = await geminiService.generate(prompt, 'projectNextSteps', userId, '/api/projects/health');
      return result || `1. Finalize current phase: ${currentPhase.phaseName}.\n2. Review pending deliverables with project lead.\n3. Verify supplier lead times for scheduled items.\n4. Send bi-weekly update report to client.`;
    } catch (e) {
...
Respond in valid JSON format:
{
  ...
  "aiRecommendation": "Recommended action to resolve"
    ...
    try {
      const result = await geminiService.generate(prompt, 'projectAlertMessage', userId, '/api/projects/alerts', true);
...
module.exports = new GeminiProjectService();
