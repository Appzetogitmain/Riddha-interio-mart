...
Return ONLY plain text text string (no quotes around output).`;
    ...
    try {
      const result = await geminiService.generate(prompt, 'quotationOpening', userId, '/api/quotations/ai-enhance');
...
Return ONLY a valid JSON object:
{
  ...
  "schedule": [
    ...
    try {
      const result = await geminiService.generate(prompt, 'quotationPaymentTerms', userId, '/api/quotations/ai-enhance', true);
...
["Term 1", "Term 2", "Term 3"]`;
    ...
    try {
      const result = await geminiService.generate(prompt, 'quotationDeliveryTerms', userId, '/api/quotations/ai-enhance', true);
...
Return ONLY a valid JSON object:
{
  ...
  "nextSteps": "1-sentence call to action"
    ...
    try {
      const result = await geminiService.generate(prompt, 'quotationSummary', userId, '/api/quotations/ai-enhance', true);
...
Return ONLY plain text string.`;
    ...
    try {
      const result = await geminiService.generate(prompt, 'quotationClosing', userId, '/api/quotations/ai-enhance');
      if (result && typeof result === 'string') return result.trim();
    } catch (e) {}
...
module.exports = new GeminiQuotationService();
