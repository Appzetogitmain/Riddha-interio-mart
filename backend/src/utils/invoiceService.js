...
const processInvoiceAsync = async (orderId, userId) => {
  try {
    console.log(
      `[INVOICE_SERVICE] Starting invoice process for order ${orderId}`,
    );
    ...
    if (!order) {
      console.error(
        `[INVOICE_SERVICE] Order not found for orderId ${orderId}`,
      );
      return null;
    }
    
    if (!user) {
      console.warn(`[INVOICE_SERVICE] User not found for order ${orderId}, skipping email but will still generate PDF.`);
    }
    ...
    // 3. Send Email to User (only if user exists and has email)
    if (user && user.email) {
      const emailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
          ...
          <br/>
          <p>Best regards,<br/><strong>Riddha Interior Mart</strong></p>
        </div>
      `;
      ...
      console.log(`[INVOICE_SERVICE] Skipped email sending (no user or email found)`);
    }
    
    return invoiceUrl;
  } catch (error) {
    console.error(
      `[INVOICE_SERVICE] Failed to process invoice for order ${orderId}:`,
      error,
    );
    throw error;
  }
