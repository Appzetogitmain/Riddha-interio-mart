...
const Navbar = () => {
  ...
  const [activeAllCat, setActiveAllCat] = useState(null);
  const [activeAllSubcat, setActiveAllSubcat] = useState(null);

  const handleCatHover = (cat) => {
    setActiveAllCat(cat);
    setActiveAllSubcat(null);
  };
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (nextCategories.length > 0) {
          setActiveAllCat(nextCategories[0]);
          setActiveAllSubcat(null);
        }
      } catch (err) {
        ...
        <div className="max-w-[1700px] mx-auto px-4 lg:px-6">
          ...
          <div className="hidden md:flex items-center justify-between py-2.5 gap-2 lg:gap-4">
            ...
            {!isCartPage && currentPath !== '/search' && (
              <div className="w-full max-w-[380px] lg:max-w-[440px] xl:max-w-[500px] px-2 mx-auto">
                <SearchBar variant="premium" />
              </div>
            )}

            {/* Seller, Bulk Order & Upgrade to Pro shortcuts */}
          ...
          <div className="flex items-center gap-4 w-full">
            ...
            <div className="relative group/cat shrink-0">
              ...
              >
                ...
                <FiChevronDown className="w-3.5 h-3.5 opacity-80 group-hover/cat:rotate-180 transition-transform duration-300 ml-1" />
              </Link>

              {/* Multi-Tier 3-Column Cascading Mega-Menu Dropdown */}
              <div className="absolute top-full left-0 pt-2 opacity-0 invisible group-hover/cat:opacity-100 group-hover/cat:visible transition-all duration-300 z-50">
                <div className="bg-white shadow-2xl border border-gray-100 rounded-2xl flex overflow-hidden h-[430px]">
                  
                  {/* 1st Tier: Main Categories List (Col 1) */}
                  <div className="w-60 py-3 bg-white border-r border-gray-100 overflow-y-auto custom-scrollbar shrink-0">
                    <div className="px-5 pb-2 mb-2 border-b border-gray-100 flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase tracking-widest text-[#189D91]">
                        1. Categories
                      </span>
                    ...
                    {categories.map((cat, i) => {
                      const currentSelected = activeAllCat || categories[0];
                      const isSelected = (currentSelected?._id || currentSelected?.name) === (cat._id || cat.name);
                      ...
                      return (
                        <Link
                          ...
                          className={`flex items-center justify-between px-5 py-2.5 text-[13px] font-bold transition-all ${
                            isSelected
                        ...
                        >
                          ...
                          {hasSubcats && (
                            <FiChevronRight className={`w-4 h-4 shrink-0 transition-transform ${isSelected ? 'translate-x-0.5 text-[#189D91]' : 'text-gray-400'}`} />
                          )}
                        </Link>
                      );
                    })}
                    <div className="mt-2 pt-2 border-t border-gray-100 px-5">
                      <Link to="/categories" className="text-xs font-bold text-[#189D91] hover:underline flex items-center gap-1">
                  ...
                  {/* 2nd Tier: Subcategories List (Col 2) */}
                  <div className="w-64 py-3 bg-slate-50/70 border-r border-gray-100 overflow-y-auto custom-scrollbar shrink-0">
                    {(() => {
                      const currentCat = activeAllCat || categories[0];
                      const subcats = currentCat?.subcategories || [];

                      return (
                        <div>
                          <div className="px-5 pb-2 mb-2 border-b border-gray-200/60 flex items-center justify-between">
                            <span className="text-[10px] font-black uppercase tracking-widest text-[#189D91]">
                              2. Subcategories
                            </span>
                          ...
                          {subcats.length > 0 ? (
                            subcats.map((sub, sIdx) => {
                              const isSubSelected = activeAllSubcat && ((activeAllSubcat._id && activeAllSubcat._id === sub._id) || activeAllSubcat.name === sub.name);
                              const hasSubSubs = sub.subsubcategories && sub.subsubcategories.length > 0;

                              return (
                                <div key={sub._id || sIdx} onMouseEnter={() => setActiveAllSubcat(sub)}>
                                  <Link
                                    to={`/category/${getCategorySlug(currentCat.name)}?sub=${encodeURIComponent(sub.name)}`}
                                    className={`flex items-center justify-between px-5 py-2.5 text-[12.5px] font-bold transition-all ${
                                      isSubSelected
                                        ? 'bg-white text-[#189D91] border-l-4 border-[#189D91] shadow-sm'
                                        : 'text-gray-700 hover:bg-white/80 hover:text-[#28a399] border-l-4 border-transparent'
                                    }`}
                                  >
                                    <span className="truncate">{sub.name}</span>
                                    {hasSubSubs && (
                                      <FiChevronRight className={`w-4 h-4 shrink-0 transition-transform ${isSubSelected ? 'translate-x-0.5 text-[#189D91]' : 'text-gray-400'}`} />
                                    )}
                                  </Link>
                                </div>
                              );
                            ...
                            <div className="p-5 text-center text-gray-400">
                              <p className="text-xs font-semibold">No subcategories</p>
                              <Link
                                to={`/category/${getCategorySlug(currentCat?.name)}`}
                                className="mt-2 text-[11px] font-bold text-[#189D91] hover:underline block"
                              >
                                View {currentCat?.name} Products &rarr;
                              </Link>
                            </div>
                          )}
                        </div>
                  ...
                  {/* 3rd Tier: Sub-subcategories List (Col 3) — Only rendered when a subcategory is hovered */}
                  {activeAllSubcat && (
                    <div className="w-72 p-5 bg-white overflow-y-auto custom-scrollbar shrink-0 border-l border-gray-100 animate-in fade-in slide-in-from-left-2 duration-200">
                      {(() => {
                        const currentCat = activeAllCat || categories[0];
                        const currentSub = activeAllSubcat;
                        const subsubs = currentSub.subsubcategories || [];

                        return (
                          <div>
                            <div className="pb-3 mb-3 border-b border-gray-100 flex items-center justify-between">
                              <div>
                                <span className="text-[10px] font-black uppercase tracking-widest text-[#189D91] block">
                                  3. Sub-subcategories
                                </span>
                                <h5 className="text-xs font-black text-gray-900 truncate mt-0.5 max-w-[170px]">
                                  {currentSub.name}
                                </h5>
                              </div>
                              <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-slate-100 text-gray-600">
                                {subsubs.length} Items
                              </span>
                            </div>

                            {subsubs.length > 0 ? (
                              <div className="space-y-1">
                                {subsubs.map((subsub, ssIdx) => {
                                  const subsubName = typeof subsub === 'string' ? subsub : subsub.name;
                                  return (
                                    <Link
                                      key={subsub._id || ssIdx}
                                      to={`/category/${getCategorySlug(currentCat.name)}?sub=${encodeURIComponent(currentSub.name)}&subsub=${encodeURIComponent(subsubName)}`}
                                      className="flex items-center justify-between px-3 py-2 text-[12px] font-semibold text-gray-700 hover:bg-[#189D91]/10 hover:text-[#189D91] rounded-xl transition-all border border-transparent hover:border-[#189D91]/20 group/ss"
                                    >
                                      <span className="truncate">&bull; {subsubName}</span>
                                      <FiChevronRight className="w-3.5 h-3.5 text-gray-300 group-hover/ss:text-[#189D91] group-hover/ss:translate-x-0.5 transition-all" />
                                    </Link>
                                  );
                                })}
                              </div>
                              ...
                              <div className="py-8 text-center text-gray-400">
                                <p className="text-xs font-semibold">No direct sub-types</p>
                                <Link
                                  to={`/category/${getCategorySlug(currentCat.name)}?sub=${encodeURIComponent(currentSub.name)}`}
                                  className="mt-3 inline-block px-3 py-1.5 bg-[#28a399] text-white rounded-lg text-xs font-bold shadow-sm hover:bg-[#003d33] transition-colors"
                                >
                                  Explore {currentSub.name}
                                </Link>
                              </div>
                            )}
                          </div>
