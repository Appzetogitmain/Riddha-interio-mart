...
const LoginPage = () => {
  ...
  const theme = {
    primaryBtn: isDelivery
      ? "bg-[#2A458A] hover:opacity-90"
      : "bg-[#189D91] hover:opacity-90",
    accentText: isDelivery ? "text-[#2A458A]" : "text-[#189D91]",
    focusBorder: isDelivery
      ? "focus:border-[#2A458A] focus:ring-[#2A458A]/5"
      : "focus:border-[#189D91] focus:ring-[#189D91]/5",
    iconFocus: isDelivery
      ? "group-focus-within:text-[#2A458A]"
      : "group-focus-within:text-[#189D91]",
    checkboxBg: isDelivery
      ? "bg-[#2A458A] border-[#2A458A]"
      : "bg-[#189D91] border-[#189D91]",
          ...
          <div className="flex-1 md:w-1/2 w-full flex flex-col justify-center py-6 px-6 md:px-10 xl:px-12 items-center md:items-start overflow-y-auto gap-3">
            ...
            {error && (
              <div className="w-full mb-4 p-3.5 bg-rose-50 border border-rose-100 rounded-xl text-center md:text-left">
                <span className="text-rose-600 text-[10px] font-bold uppercase tracking-wider leading-relaxed">
                  {error}
                </span>
                {unverifiedEmail && (
                  <button
                    onClick={handleResendAndVerify}
                    className="block mt-2 mx-auto md:mx-0 bg-rose-600 text-white px-4 py-1 rounded-lg text-[9px] font-bold uppercase tracking-widest hover:bg-rose-700"
                  >
                    Verify Email
                  </button>
                )}
            ...
            >
              ...
              <div className="flex items-center justify-between px-0.5">
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-md border flex items-center justify-center transition-all ${rememberMe ? "bg-[#189D91] border-[#189D91]" : "border-slate-200 bg-white"}`}
                  >
                    {rememberMe && (
                ...
                <button
                  ...
                  className="text-[10px] font-bold text-[#189D91] hover:underline"
                >
                  Forgot?
                </button>
              </div>
          ...
          >
            ...
            <div className="space-y-3 text-center max-w-[460px]">
              ...
              <h2 className="text-[2.1rem] xl:text-4xl font-black text-slate-950 tracking-tight leading-[1.02]">
                India's Premium <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-800 via-slate-700 to-[var(--color-primary)] font-black">
                  Interior & Design Hub
                </span>
              </h2>

              <p className="text-slate-700 text-sm xl:text-base font-medium leading-relaxed max-w-md mx-auto">
                Curating premium architectural collection of luxury fittings and
                design setups.
              </p>
            </div>
          </motion.div>
        ...
        <div className="flex w-full lg:flex-[0.92] flex-col justify-center bg-transparent relative px-6 sm:px-12 lg:px-16 py-12 min-h-screen lg:min-h-0 overflow-y-auto">
          <div className="w-full max-w-[460px] mx-auto lg:mx-0 lg:max-w-none">
            ...
            <div className="mb-6 flex items-center justify-between gap-4 max-w-[460px]">
              ...
              <Link
                to="/"
                className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors border border-slate-100 shrink-0"
              >
                <FiArrowLeft size={16} />
              </Link>
            </div>
            ...
            <AnimatePresence>
              {error && (
                ...
                >
                  <span className="text-rose-600 text-[10px] font-bold uppercase tracking-wider text-center leading-relaxed">
                    {error}
                  </span>
                  {unverifiedEmail && (
                    <button
                      onClick={handleResendAndVerify}
                      className="bg-rose-600 text-white px-5 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-rose-700 transition-colors"
                    >
                      Verify Email
                    </button>
                  )}
            ...
            <form onSubmit={handleLogin} className="space-y-4 max-w-[460px]">
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500 ml-0.5">
                    Email Address
                  </label>
                  <div className="relative group">
                    <FiUser
                      className={`absolute left-4 top-1/2 -translate-y-1/2 text-slate-350 ${theme.iconFocus} transition-colors`}
                      size={16}
                    />
                    <input
                      type="text"
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center px-0.5">
                    ...
                    <button
                      type="button"
                      onClick={() => navigate("/forgot-password")}
                      className={`text-[10px] font-black ${theme.accentText} uppercase tracking-[0.18em] hover:underline`}
                    >
                      Forgot?
                    </button>
                  </div>
                  <div className="relative group">
                    <FiLock
                      className={`absolute left-4 top-1/2 -translate-y-1/2 text-slate-355 ${theme.iconFocus} transition-colors`}
                      size={16}
                    />
                    <input
                      type={showPassword ? "text" : "password"}
              ...
              <div className="flex items-center gap-2.5 px-0.5 pt-1">
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-md border flex items-center justify-center transition-all ${rememberMe ? theme.checkboxBg : "border-slate-200 bg-white group-hover:border-slate-300"}`}
                  >
                    {rememberMe && (
                      <FiCheck className="text-white text-[10px] stroke-[4]" />
                    )}
                  </div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 group-hover:text-slate-600 transition-colors">
                    Remember Me
                  </span>
                </button>
              </div>
